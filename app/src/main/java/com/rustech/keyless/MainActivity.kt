...
class MainActivity : AppCompatActivity() {
    ...
    ) { /* User can retry the relevant switch if something was denied. */ }
    ...
    /** Shows/hides the stats row and updates card stroke color based on running state. */
    private fun updateBroadcastCard(running: Boolean) {
        ...
        binding.tvBroadcastStatus.textColor(
            if (running) R.color.accent_green else R.color.text_secondary
        )
        // Update card stroke to glow green when active
        binding.cardBroadcast.strokeColor = ContextCompat.getColor(
            this,
            if (running) R.color.accent_green_dim else R.color.accent_green_border
        )
    }

    private fun TextView.textColor(colorRes: Int) {
    ...
    private fun pruneStaleBeacons() {
        ...
        beaconMap.filterValues { it.lastSeen < cutoff }.keys.forEach { beaconMap.remove(it) }
    }
...
// Extension to avoid repeating context reference
private fun android.widget.TextView.textColor(colorRes: Int) {
    setTextColor(ContextCompat.getColor(context, colorRes))
}
